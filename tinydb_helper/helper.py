...
class Table:
    def __init__(self, db: TinyDB, table: str, primary_key: str = None, key_gen: callable = None) -> None:
        self.table = db.table(table)
        self.primary_key = f'{table}_id' if primary_key == None else primary_key
        self.key_gen = default_key_gen if key_gen == None else key_gen
        self.search = Query()
    ...
    def insert(self, item: dict, id: str = None) -> str:
        id = self.key_gen() if id == None else id
